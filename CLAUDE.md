...
- Tầng AI (✦) chạy theo «cách c»: người dùng tự dán khoá API. Từ v10.3 nhà cung cấp
  là OpenRouter (openrouter.ai, model `google/gemini-3.7-flash` — hằng MODEL_AI trong
  app/src/lib/ai.js). Khoá nằm ở localStorage riêng `ke-hoach-du-lich-api-key` —
  TÁCH BIỆT với sổ chuyến đi. TUYỆT ĐỐI không đưa khoá vào `kho`/backup/Supabase/repo.
